    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="px-8 py-6 border-b border-[#1C1C1C] flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-[#555] text-sm mt-0.5">Visão computacional + IA aplicada ao seu físico</p>
        </div>
        <button onClick={() => setActiveTab('chat')}
          className="flex items-center gap-2 text-xs text-[#555] hover:text-[#999] border border-[#1C1C1C] hover:border-[#333] rounded-xl px-4 py-2.5 transition-all">
          ...
          <motion.div key="analysis" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  {/* Tips */}
                  <div className="mt-6 rounded-xl bg-[#111] border border-[#1C1C1C] p-5 card-lift">
                ...
                <div>
                  {!result ? (
                    <div className="h-full flex items-center justify-center text-center py-20">
                      <div>
                        <div className={`relative w-20 h-20 rounded-2xl bg-[#111] border flex items-center justify-center mx-auto mb-4 overflow-hidden ${loading ? 'border-[#E8002D]/30 text-[#E8002D]' : 'border-[#1C1C1C] text-[#333]'}`}>
                          <IconScan />
                    ...
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                      ...
                      {result.overall_score !== undefined && (
                        <div className="grid grid-cols-2 gap-3">
                          {/* Score */}
                          <div className="rounded-2xl bg-[#111] border border-[#1C1C1C] p-5 flex flex-col items-center justify-center card-lift">
                            <RadialScore value={result.overall_score} max={10} size={108} strokeWidth={6} color="var(--red)" label="SCORE" />
                          </div>
                          {/* BF% */}
                          <div className="rounded-2xl bg-[#111] border border-[#1C1C1C] p-5 flex flex-col items-center justify-center card-lift">
                      ...
                      {result.muscle_mass && (
                        <div className="rounded-xl bg-[#111] border border-[#1C1C1C] px-5 py-3 flex items-center justify-between">
                      ...
                      {result.strong_points && (
                        <div className="rounded-2xl bg-[#111] border border-[#1C1C1C] p-5 card-lift">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div className="sm:border-l border-[#1C1C1C] sm:pl-5">
                              <p className="text-xs font-black uppercase tracking-widest mb-3 flex items-center gap-1.5 text-[#E8002D]" style={{ fontFamily: 'var(--font-display)' }}>
                                <IconTrendUp />PRIORIDADES
                              </p>
                              <ul className="space-y-2">
                                {result.weak_points?.map((p, i) => (
                                  <li key={i} className="text-sm text-[#999] flex gap-2 leading-relaxed"><span className="text-[#E8002D] mt-0.5 flex-shrink-0"><IconTrendUp /></span>{p}</li>
                                ))}
                              </ul>
                            </div>
                          </div>
                      ...
                      {result.motivational_message && (
                        <div className="rounded-xl bg-[#E8002D]/5 border border-[#E8002D]/15 p-4 card-lift">
                          <p className="text-sm text-[#E8E8E8] italic leading-relaxed">{result.motivational_message}</p>
                        </div>
                      )}

                      {result.training_plan && (
                        <details className="rounded-2xl bg-[#111] border border-[#1C1C1C] overflow-hidden card-lift">
